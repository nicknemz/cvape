...
GuiLibrary["MainGui"].ScaledGui.ClickGui.Version.Text = "LcWare"
GuiLibrary["MainGui"].ScaledGui.ClickGui.MainWindow.TextLabel.Text = "LCWare"
GuiLibrary["MainGui"].ScaledGui.ClickGui.Version.Version.Text = "LcWare"
GuiLibrary["MainGui"].ScaledGui.ClickGui.Version.Position = UDim2.new(1, -175 - 20, 1, -25)
infonotify("LcWare", "Loaded Fine", 5)
...
infonotify("Vape", "Credit scepty#2087", 15)
...
local LandmindeAura = COB("Combat", {
	["Name"]  = "LCWareSwordDelay",
	["Function"] = function(callback)
		if callback then
			pcall(function()
				ScriptSettings.MiniAura = true
				while task.wait(0.15) do
					if not ScriptSettings.MiniAura == true then return end
					for i,v in pairs(game:GetService("Players"):GetChildren()) do
...
local InviteCrash = COB("Utility", {
	...
	COB("Utility", {
		...
		HoverText = "On Keybind better version soon"
	})
	...
	COB("Blatant", {
        ...
        HoverText = "330 members congrats!"
    })
	...
	game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.BedwarsPurchaseItem:InvokeServer({    ["shopItem"] = {        ["currency"] = "iron",        ["itemType"] = "wool_white",        ["category"] = "Blocks",        ["price"] = 8,        ["amount"] = 16,    },})
				end) 
			end
		end,
		HoverText = "idk what i maded for keybind"
	})

	COB("Utility", {
		Name = "BuyDiamondPickaxe",
		Function = function(callback) 
			if callback then
				pcall(function() 
					-- small feature v3.7
	game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.BedwarsPurchaseItem:InvokeServer({
		...
		HoverText = "recreation like autobuy"
	})
...
--Fucking skiddeer die
game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.BedwarsPurchaseItem:InvokeServer({    ["shopItem"] = {        ["ignoredByKit"] = {            [1] = "barbarian",            [2] = "dasher",        },        ["lockAfterPurchase"] = true,        ["itemType"] = "iron_sword",        ["amount"] = 1,        ["price"] = 70,        ["category"] = "Combat",        ["disabledInQueue"] = {            [1] = "tnt_wars",        },        ["currency"] = "iron",    },})
            end) 
        end
    end,
    HoverText = "buying iron sword"
})
...
game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.BedwarsPurchaseItem:InvokeServer({
    ...
    HoverText = "SmallAutoBuyRecreaion3"
})


infonotify("LcWare", "Praise Scepty", 20)
